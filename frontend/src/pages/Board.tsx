...
const COLUMNS: { id: KanbanStage; label: string; accent: string; bg: string; dot: string }[] = [
  { id: 'applied',   label: 'Applied',   accent: 'text-slate-700',   bg: 'bg-slate-50  border-slate-200',  dot: 'bg-slate-400' },
  { id: 'interview', label: 'Interview', accent: 'text-violet-700',  bg: 'bg-violet-50 border-violet-200', dot: 'bg-violet-500' },
  { id: 'offer',     label: 'Offer',     accent: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200', dot: 'bg-emerald-500' },
  { id: 'rejected',  label: 'Rejected',  accent: 'text-red-600',     bg: 'bg-red-50 border-red-200',       dot: 'bg-red-400' },
]
...
function DraggableCard({ app, isDragging }: { app: Application; isDragging?: boolean }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`bg-white rounded-xl border border-slate-100 p-3.5 shadow-sm transition-all ${
        isDragging ? 'opacity-40 scale-95' : 'hover:shadow-md hover:border-indigo-100'
      }`}
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-slate-900 leading-tight truncate">{app.job?.title ?? '—'}</p>
          <p className="text-xs text-slate-500 mt-0.5 truncate">{app.job?.company}</p>
          {app.job?.location && (
  ...
  return (
    <div className="flex flex-col min-w-[240px] flex-1">
      <div className={`flex items-center justify-between px-3 py-2.5 rounded-xl border mb-3 ${column.bg}`}>
        ...
        <span className={`text-xs font-bold ${column.accent} opacity-60 bg-white/60 rounded-full w-5 h-5 flex items-center justify-center`}>
          {apps.length}
        </span>
      </div>
      <div
        ...
        <div className="flex gap-4">
          {COLUMNS.map((c) => (
            <div key={c.id} className="flex-1 min-w-[240px]">
              <div className="h-10 bg-slate-100 rounded-xl animate-pulse mb-3" />
  ...
  return (
    <div className="p-8 h-full flex flex-col">
      <div className="mb-6">
        ...
        <p className="text-slate-500 text-sm">Drag cards to track your application progress.</p>
      </div>
      ...
      {apps.length === 0 ? (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-slate-400 text-sm mt-1">Apply to jobs and track your progress here.</p>
