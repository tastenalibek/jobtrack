        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={handleSubmit} disabled={loading}
              className="flex-1 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 disabled:opacity-50 shadow-sm shadow-indigo-500/20">
...
export default function AdminJobsPage() {
  ...
  const updateMut = useMutation({
    ...
    onSuccess: (job) => {
      qc.invalidateQueries({ queryKey: ['admin-jobs'] })
      success(`"${job.title}" updated.`)
    },
    onError: () => error('Failed to update job.'),
  })
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 shadow-sm shadow-indigo-500/20"
        >
      ...
      {modal && (
        <JobFormModal initial={modal} onClose={() => setModal(null)} onSave={handleSave} />
      )}
    </div>
  )
}
